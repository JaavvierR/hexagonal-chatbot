...
const GEMINI_API_KEY = process.env.GEMINI_API_KEY || 'AIzaSyDRivvwFML1GTZ_S-h5Qfx4qP3EKforMoM'; // ¡Reemplaza con tu clave o usa .env!
...
const CATALOG_PATH = path.join(__dirname, '..', 'catalogo_.pdf'); // Mantener si catalogo_.pdf está en la raíz
...
async function facebookMessengerBot(targetChatId = null) {
    /**
    ...
    try {
        ...
        try {
            ...
            const uiService = new SeleniumUiService(driver); // Usar la interfaz
            ...
            const processedMessageIds = new Set(); // To track processed messages by ID
            // Function to process the latest message in the current chat
            async function processLatestMessage(driver) {
                try {
                    ...
                    const messageElements = await driver.findElements(By.xpath("//div[@role='main']//div[@role='row'] | //div[@role='main']//div[contains(@class, 'x1iorvi4') and contains(@class, 'x1pi30zi')]" // Intenta con role='row' o clases comunes de burbujas de mensaje
                    ));
            ...
            while (true) {
                try {
                    if (targetChatId) {
                        ...
                        const unreadConversations = await driver.findElements(By.xpath("//div[contains(@aria-label, 'unread') or contains(@aria-label, 'no leído') or contains(@aria-label, 'New message')]"));
